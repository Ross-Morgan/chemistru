...
impl CovalentBond {
    ...
    pub fn new_primitive(v: u8) -> Self {
        Self(
            v.try_into()
                .expect("Covalent bond cannot have order of zero"),
        )
    }
...
impl CovalentMolecule {
    pub fn formula(&self) -> String {
        ...
        self.inner.nodes.values().cloned().for_each(|e| {
            buf.entry(e.symbol()).and_modify(|c| *c += 1).or_insert(1);
        });

        let mut v = buf.into_iter().collect::<Vec<_>>();
